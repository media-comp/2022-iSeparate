...
def splitMusicData(dataset_root_path, output_root_path="./music_data", type="train"):
    print('Split Music Segments: Start processing...')
    DATASET_PATH = os.path.join(dataset_root_path, "Dev" if type == "train" else "Test")
    resample_data_path = os.path.join(dataset_root_path, "Dev_16k" if type == "train" else "Test_16k")
    split_data_path = os.path.join(output_root_path, type)
    if not os.path.exists(resample_data_path):
        os.makedirs(resample_data_path)
    if not os.path.exists(split_data_path):
        os.makedirs(split_data_path)
    segment_len = 10 # The splited audio length
    start_music_index = 0 # Update index before using
    need_resample = True
    need_split = True
