...
from iSeparate.losses.loss_switcher import get_loss_fn
        ...
        for batch_idx, batch in enumerate(eval_loader):
            ...
            if display_idx == batch_idx:
                ...
                if len(output.shape) == 4:
                    ...
                    y_stft = model.time_domain_wrapper.transform(
                        y_wav[0], patch_length=args.val_patch_length, return_magnitude=False
                    )
                    y_hat_phase = y_stft.angle()
                    y_hat_wav = model.time_domain_wrapper.inverse_transform(
                        magnitude=y_hat_mag, phase=y_hat_phase
                    )
            if args.debug:
                break
        tb_logger.log_audio(
            ...
            y_wav.cpu(),
            y_hat_wav.cpu(),
            args.data_loader_args["train"]["target_sources"],
        )
...
def main():
    ...
    if args.use_stats:
        if args.stats_path is None or not os.path.exists(args.stats_path):
            ...
            mus = musdb.DB(
                root=args.data_loader_args["train"]["data_root"], subsets="train", is_wav=True
            )
            input_mean, input_scale = get_statistics(args, mus)
            stats_path = os.path.join(
                args.data_loader_args["train"]["data_root"], "musdb_train.stats"
            )
            if local_rank == 0:
                torch.save({"mean": input_mean, "std": input_scale}, stats_path)
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer,
        step_size=args.scheduler_step_epoch,
...
if __name__ == "__main__":
    ...
    #               --config-file configs/demucs/demucs_config.yaml
