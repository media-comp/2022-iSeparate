...
@torch.no_grad()
def separate(target_models, audio, niter, cpu_only, softmask):
    if len(target_models) == 1:
        assert (
            "vocals" in target_models
        ), "For calculating scores, at least vocals target is necessary!"
    device = "cuda" if torch.cuda.is_available() and not cpu_only else "cpu"

    mix_audio = audio.to(device)
    if len(mix_audio.shape) == 2:
        mix_audio = mix_audio.unsqueeze(
            0
        )  # add batch dimension if only single audio tensor is provided
    ...
    if nb_sources < 4:
        ...
        if "vocals" in target_models:
            ...
            estimate_accompaniment = norbert.contrib.residual_model(
                estimates["vocals"][..., np.newaxis], mix_stft.cpu().numpy()
            )
            estimates["accompaniment"] = estimate_accompaniment[..., 1]
            estimates_tensor = torch.cat(
                [estimates_tensor, torch.from_numpy(estimate_accompaniment[..., 1:])], dim=-1
            )

    Y = norbert.wiener(
        estimates_tensor.numpy(), mix_stft.cpu().numpy(), use_softmask=softmask, iterations=niter
    )

    for i, target in enumerate(estimates.keys()):
        estimates[target] = istft_fn(
            torch.from_numpy(Y[..., i]).permute(2, 1, 0)
        )  # t f c --> c f t
