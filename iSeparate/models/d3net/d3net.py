...
class D2Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.num_layers > 1:
            tensor_splits = [
                t.clone() for t in torch.split(out, split_size_or_sections=self.splits, dim=1)
            ]
            for i, (split, conv) in enumerate(zip(tensor_splits, self.convs)):
                out = conv(split)
                for j in range(self.num_layers - i - 1):
                    tensor_splits[j + 1 + i] += out[
                        :, j * self.growth_rate : (j + 1) * self.growth_rate
                    ]
...
class D3block(nn.Module):
    def __init__(
        self, in_channels, growth_rate, num_layers, num_blocks, kernel_size, padding, num_outs
    ):
        ...
        self.init_block = D2Block(
            in_channels, growth_rate * num_blocks, kernel_size, padding, num_layers, num_outs
        )
...
class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.tconv = nn.ConvTranspose2d(
            in_channels=in_channels, out_channels=out_channels, kernel_size=(2, 2), stride=(2, 2)
        )
...
class MD3Net(nn.Module):
    def __init__(
        self, in_channels, growth_rates, num_layers, num_blocks, kernel_size, padding, num_outs
    ):
        ...
        for k, n_layers, n_blocks in zip(
            growth_rates[:num_downsamples],
            num_layers[:num_downsamples],
            num_blocks[:num_downsamples],
        ):
            self.downsample_blocks.append(
                D3block(in_channels, k, n_layers, n_blocks, kernel_size, padding, num_outs)
            )
        ...
        for i, (k, n_layers, n_blocks) in enumerate(
            zip(
                growth_rates[num_upsamples + 1 :],
                ...
                nn.ModuleDict(
                    {
                        ...
                        "D3block": D3block(
                            n_channels_concat, k, n_layers, n_blocks, kernel_size, padding, num_outs
                        ),
                    }
                )
            )
...
class SpectrogramFunction(nn.Module):
    def __init__(self, n_fft, hop_length, win_length):
        ...
        self.stft = torchaudio.transforms.Spectrogram(
            n_fft=n_fft, win_length=win_length, hop_length=hop_length, power=None, center=True
        )
        self.istft = torchaudio.transforms.InverseSpectrogram(
            n_fft=n_fft, win_length=win_length, hop_length=hop_length, center=True
        )
...
class MMD3Net(nn.Module):
    def __init__(
        ...
        assert (
            len(band_split_idxs) + 1
            == len(growth_rates["bands"])
            == len(num_layers["bands"])
            == len(num_blocks["bands"])
            == len(num_outs["bands"])
            == len(init_channels["bands"])
        )
        ...
        self.full_net = nn.Sequential(
            nn.Conv2d(
                audio_channels, init_channels["full"], kernel_size=kernel_size, padding=padding
            ),
            MD3Net(
                init_channels["full"],
        ...
        # band networks
        max_final_growth_rate = max(
            [k[-1] * n for k, n in zip(growth_rates["bands"], num_outs["bands"])]
        )
        self.band_nets = nn.ModuleList()
        for i, (init_features, growth_rate, num_layer, num_block, num_out) in enumerate(
            zip(
                init_channels["bands"],
                growth_rates["bands"],
                num_layers["bands"],
            ...
            init_conv = nn.Conv2d(
                audio_channels, init_features, kernel_size=kernel_size, padding=padding
            )
            d3net = MD3Net(
                init_features, growth_rate, num_layer, num_block, kernel_size, padding, num_out
            )
            if max_final_growth_rate > growth_rate[-1] * num_out:
                feature_match_layer = BNReLUConv(
                    growth_rate[-1] * num_out,
        ...
        self.gate_out = nn.Sequential(
            nn.Conv2d(in_channels, audio_channels, kernel_size=(1, 1), stride=(1, 1)), nn.Sigmoid()
        )
    ...
    def forward(self, x, y=None, return_input_spec=False, patch_length=-1):
        ...
        x = self.time_domain_wrapper.transform(
            x, patch_length=patch_length, return_magnitude=True
        ).transpose(2, 3)
        ...
        if y is not None:
            ...
            # y = y.squeeze(1)
            y = self.time_domain_wrapper.transform(
                y, patch_length=patch_length, return_magnitude=True
            ).transpose(3, 4)
    ...
    def separate(self, audio, hop_length=None):
        spect = self.time_domain_wrapper.transform(audio, return_magnitude=True).transpose(
            2, 3
        )  # b, c, t, f
        b, c, t, f = spect.shape
        ...
        assert (
            hop_ratio.is_integer()
        )  # the hop ratio should be smaller than patch length and exactly divisible
        ...
        fold_params = dict(
            kernel_size=(self.patch_length, f), dilation=1, padding=0, stride=(hop_length, f)
        )

        chunks = (
            spect_padded.unfold(2, self.patch_length, hop_length)
            .permute(0, 1, 4, 3, 2)
            .contiguous()
        )
        ...
        for chunk_idx in range(nb_chunks):
            ...
            out = (
                self.process_spects(chunk).view(b, -1) / hop_ratio
            )  # process and reshape to b, c*patch_length*f
        ...
        final_output = torch.nn.Fold(output_size=spect_padded.shape[2:], **fold_params)(
            processed_chunks
        )  # b, c, patch_length, f
