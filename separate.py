...
from iSeparate.models.model_switcher import get_separation_funcs
from iSeparate.utils.common_utils import preprocess_audio
...
def separate(audio_file_path, eval_config_path, outdir="separation_results"):
    ...
    if cfg["model_name"] != "D3Net":
        print(
            "Currently only D3Net model is available for download,"
    ...
    estimates = separator(model, audio, **cfg["eval_params"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--config", type=str, required=True, help="path to the evaluation config file"
    )
    parser.add_argument(
        "-i", "--input", type=str, required=True, help="path to the audio file to be separated"
    )
    parser.add_argument(
        "-o",
        "--outdir",
        type=str,
        default="separation_results",
